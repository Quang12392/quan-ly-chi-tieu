...
const safePayload={...records[0],date:'2026-09-14',amount:76543};
const safeRequestId='request_abcdef1234567890';
const firstSafeWrite=await api.createTransaction(safePayload,safeRequestId);
const recoveredSafeWrite=await api.syncTransaction(safePayload,safeRequestId);
assert.equal(recoveredSafeWrite.id,firstSafeWrite.id,'Safe synchronization reuses the original transaction');
assert.equal(server.api('getTransactionsPage',{from:'2026-09-14',through:'2026-09-14'}).items.filter(tx=>tx.id===firstSafeWrite.id).length,1);
